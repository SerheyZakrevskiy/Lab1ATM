    ...
    public class AutomatedTellerMachine
    {
       ...
       BankOperationsEventArgs e);
        ...
        public void Deposit(double sum, Account[] account, int user)
        {
            ...
            AtmBalance += sum;
            Notify?.Invoke(this, new BankOperationsEventArgs($"На рахунок зараховано
           { sum } UAH", sum));
        }
                ...
                {
                    {
                        account[user].Balance -= sum;
                        AtmBalance -= sum;
                        Notify?.Invoke(this, new BankOperationsEventArgs($"Знято {sum}
                        UAH", sum));
                    }
                }
                else
                {
                    Notify?.Invoke(this, new BankOperationsEventArgs("Недостатньо
                   коштів на рахунку", sum));
                }
            }
            else Notify?.Invoke(this, new BankOperationsEventArgs("Технічні проблеми.
           Будь ласка, спробуйте пізніше.", sum));
        }
                ...
                if (check == false)
                {
                    for (int j = 0; j < Account.usercount; j++)
                    {
                        if (account[j].CardNumber == cardNumber)
                        {
                            ...
                            Notify?.Invoke(this, new BankOperationsEventArgs($"{sum}
                            UAH було перераховано на картку { cardNumber }", sum));
                        return true;
                        }
                    }
                    if (check == false) Notify?.Invoke(this, new
                    BankOperationsEventArgs($"Картку {cardNumber} не знайдено", sum));
                }
            }
            else
